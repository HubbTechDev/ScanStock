...
import { Package, Search, Plus, CheckCircle, Calendar, ArrowUp, ArrowDown, Camera, X } from 'lucide-react-native';
...
export default function InventoryScreen() {
  ...
  // Sold/Shipped items (completed status)
  const soldItems = items.filter((item) => item.status === 'completed');

  const currentItems = activeTab === 'active' ? activeItems : soldItems;
    ...
    <View className="flex-1 bg-neutral-900">
      ...
      <SafeAreaView className="flex-1" edges={['top']}>
        ...
        <View className="px-5 pt-4 pb-4 flex-row items-center justify-between">
          ...
          <Pressable
            className="bg-cyan-500 w-12 h-12 rounded-xl items-center justify-center active:opacity-80"
        ...
        <View className="px-5 mb-4">
          <View className="bg-slate-800/60 rounded-xl p-1 flex-row border border-slate-700/50">
            ...
            <Pressable
              className={`flex-1 py-2.5 rounded-lg items-center ${
                activeTab === 'sold' ? 'bg-emerald-500' : ''
              }`}
              onPress={() => setActiveTab('sold')}
            >
              <Text className={`font-semibold ${activeTab === 'sold' ? 'text-white' : 'text-slate-400'}`}>
        ...
        <View className="px-5 mb-4">
          <View className="flex-row items-center gap-2">
            ...
            <View className="flex-1 bg-slate-800/60 rounded-xl flex-row items-center px-4 border border-slate-700/50">
              ...
              <TextInput
                ...
                placeholder={`Search ${activeTab === 'active' ? 'active' : 'sold'} items...`}
          ...
          {photoSearchResults.length > 0 && (
            <Pressable
              className="mt-2 bg-cyan-500/20 rounded-lg px-3 py-2 flex-row items-center justify-between"
        ...
        >
          ...
          >
            <View className="bg-slate-800 rounded-2xl w-72 overflow-hidden border border-slate-700">
              ...
              <Pressable
                className={`px-5 py-4 flex-row items-center justify-between ${sortOrder === 'newest' ? 'bg-cyan-500/20' : ''}`}
                onPress={() => handleSortOrderChange('newest')}
              >
                <View className="flex-row items-center">
                  <ArrowDown size={18} color={sortOrder === 'newest' ? '#06B6D4' : '#94A3B8'} />
                  <Text className={`ml-3 ${sortOrder === 'newest' ? 'text-cyan-400 font-semibold' : 'text-slate-300'}`}>
                    Newest First
                  </Text>
                </View>
                {sortOrder === 'newest' && <CheckCircle size={18} color="#06B6D4" />}
              </Pressable>
              <Pressable
                className={`px-5 py-4 flex-row items-center justify-between ${sortOrder === 'oldest' ? 'bg-cyan-500/20' : ''}`}
                onPress={() => handleSortOrderChange('oldest')}
              >
                <View className="flex-row items-center">
                  <ArrowUp size={18} color={sortOrder === 'oldest' ? '#06B6D4' : '#94A3B8'} />
                  <Text className={`ml-3 ${sortOrder === 'oldest' ? 'text-cyan-400 font-semibold' : 'text-slate-300'}`}>
                    Oldest First
                  </Text>
                </View>
                {sortOrder === 'oldest' && <CheckCircle size={18} color="#06B6D4" />}
              </Pressable>
            </View>
          </Pressable>
        </Modal>
            ...
            <SafeAreaView className="flex-1">
              ...
              {!permission?.granted ? (
                <View className="flex-1 justify-center items-center px-8">
                  ...
                  <Pressable
                    className="bg-cyan-500 rounded-xl px-8 py-4 mt-6 active:opacity-80"
                  ...
                  <Pressable
                    ...
                    onPress={pickImage}
                  >
                    <Text className="text-cyan-400 font-semibold">Choose from Library Instead</Text>
                  </Pressable>
                </View>
              ) : (
                    ...
                    <View style={{ flex: 1, justifyContent: 'flex-end', paddingBottom: 40 }}>
                      <View className="flex-row items-center justify-center gap-6">
                        ...
                        <Pressable
                          ...
                          onPress={takePhoto}
                        >
                          <View className="w-16 h-16 rounded-full bg-cyan-500" />
                        </Pressable>
              ...
              {activeTab === 'active' ? (
                <>
                  ...
                  <Text className="text-slate-400 mt-3 text-center">
                    {searchQuery ? 'No matching items' : 'No active items in inventory'}
                  </Text>
                  {!searchQuery && (
                    <Pressable
                ...
                <>
                  ...
                  <Text className="text-slate-400 mt-3 text-center">
                    {searchQuery ? 'No matching items' : 'No sold items yet'}
                  </Text>
            ...
            filteredItems.map((item) => (
              ...
              >
                ...
                <View className="flex-1 ml-4">
                  ...
                  {item.platform ? (
                    <Text className="text-cyan-400 text-xs mt-2">{item.platform}</Text>
                  ) : null}
                </View>
              </Pressable>
            ))
